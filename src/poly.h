...
/** Forward declaration struktury jednomianu */
typedef struct Mono Mono;

/**
 * Struktura przechowująca wielomian
 * Wilomian jest stały, gdy liczba jednomianów wynosi 0.
 * W przeciwnym razie pole `constant_value` ma w sobie liczbę 1,
 * zaś tablica jednomianów, z których skłąda się wilomian jest niepusta.
 */
typedef struct Poly {
    poly_coeff_t constant_value; ///< wartość wielomianu, gdy jest stały
    unsigned monos_count; ///< liczba jednomianów
...
static inline Poly PolyFromCoeff(poly_coeff_t c) {
    return (Poly) { .constant_value = c, .monos_count = 0, .monos = NULL};
}
...
static inline bool PolyIsCoeff(const Poly *p) {
    if(p->monos_count == 0) return true;
    else return false;
}
...
static inline bool PolyIsZero(const Poly *p) {
    if(PolyIsCoeff(p) && p->constant_value == 0) return true;
    else return false;
}
...
/**
 * Zamienia i-tą zmienną wielomianu p, na i-ty element
 * tablicy x.
 * @param p wielomian
 * @param count długość tablicy x
 * @param x tablica wielomianów do wkomponowania
 * @return
 */
